...
{ 
    public partial class OrderMasterForm_E : Form
    {
        ...
        private void ComboBoxItemCheck()
        {
            for (int i = 0; i < MainForm.dc.Shipment.Length; i++)
            {
                ...
                //if (MainForm.ShipmentData[i] != modelData[2])
                {
                    ...
                    {
                        cbbShipment.Items.Add(MainForm.dc.Shipment[i] + " - [기존]");
                        cbbShipment.SelectedIndex = i;
                    }
                    else if (MainForm.dc.Shipment[i] == modelData[3])
                    {
                        cbbShipment.Items.Add(MainForm.dc.Shipment[i] + " - [적용]");
                    }
                    else
                    {
        ...
        {
            string[] splitData = cbbShipment.Text.Split('-');
        ...
        private void btnChangeApply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string[] ComboBoxTextSplit = cbbShipment.Text.Split('-');
                string[,] resultData; //, resultData2;
                
                splitData = cbbShipment.Text.Split('-');
                ...
                {
                    MainForm.Guna2Msg(this, "오류", "마스터 기준 출하지[" + oldShilmentData + "]와 마지막 등록된 출하지[" + resultData[resultData.GetLength(0) - 1, 1] + "]가 동일합니다\n\n ※ '" + txtModelName.Text + "' 품번의 출하지 변경 이력을 확인하세요");
                    return;
                }
                else if (resultData.GetLength(0) == 0 && splitData.Length > 1 && splitData[1].Trim() == "[기존]")
                {
                    MainForm.Guna2Msg(this, "오류", "첫 등록은 마스터 기준 출하지[" + oldShilmentData + "]와 동일하게 등록할 수 없습니다");
                    return;
                }
#if !DEBUG
                else if (txtContents.Text.Length < 10)
                {
                    MainForm.Guna2Msg(this, "오류", "등록 내용을 10자 이상 입력해주세요");
                    return;
                }
#endif
